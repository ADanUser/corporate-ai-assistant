...
6. **Поведение проверяется датасетом, а не глазами.** 36 eval-кейсов
   сверяют траекторию запроса; две метрики безопасности блокируют релиз.
...
|---|---|---|---|---|
| `create_task` | создаёт задачу в Google Sheets (реальная запись, не мок) | high | все | **нужен** |
| `check_vacation_balance` | остаток отпуска сотрудника (демо-данные) | low | все | не нужен |

`create_task` — единственный write-инструмент, и он бьёт в настоящую
внешнюю систему: после approve строка добавляется в Google Sheets через
сервисный аккаунт (`app/sheets.py`). Побочный эффект отделён от текста
ответа тем же принципом, что и approval — единственное место, которое
можно посчитать в evaluation. Настройка — две переменные окружения,
см. `.env.example`.
...
Смысл в том, где проходит граница ошибки. Обойти approval можно только
**соврав про риск** в карточке — это осознанное действие. Забывчивость
при добавлении инструмента к дыре не приводит.
...
app/
  ...
  llm.py            — единая точка доступа к языковой модели, ретраи
...
evals/
  cases.py          — eval-датасет: 36 кейсов с ожидаемой траекторией
  runner.py         — прогон датасета, scorecard, release gate
tests/
  test_security.py  — быстрые тесты детерминированных слоёв (без модели)
.github/workflows/
  tests.yml         — прогон быстрых тестов на каждый push
demo.py             — десять демо-сценариев одной командой
pytest.ini          — корень проекта в sys.path для pytest
```
...
Слои не дублируют друг друга — каждый закрывает свой класс угроз. Это
проверено экспериментально: при отключённом фильтре документов по роли
конфиденциальные документы попадали в выдачу, и ни один другой слой их
не перехватывал (см. раздел Evaluation).

Порядок действий при инциденте — в [RUNBOOK.md](RUNBOOK.md).
...
Проверки разделены на два уровня — по тому, детерминирован ли проверяемый код.

| Уровень | Что проверяет | Модель | Когда | Время |
|---|---|---|---|---|
| `pytest -q` | права, реестр инструментов, детектор инъекций, kill switch | не нужна | каждый push, в CI | < 1 сек |
| `python -m evals.runner` | траекторию запроса целиком | нужна | вручную перед релизом | ~5 мин |

Деление не ради экономии квоты, а следствие архитектуры: раз решения
принимает код, а не промпт, то и проверять их можно без промпта. Все
семь слоёв защиты — обычные функции Python, их результат зависит только
от входа.

Обращения к модели недетерминированы: одна и та же формулировка между
прогонами может быть классифицирована по-разному. Такой тест в CI красил
бы билд без связи с состоянием кода, а красный статус, которому не верят,
хуже отсутствующего. Поэтому в CI гоняется только то, где красный
означает ровно одно — сломан код.

### Полный прогон траектории

Проверяется не текст ответа, а **траектория**: куда пошёл запрос, какой
документ нашёлся, какой инструмент выбран, был ли approval. Формулировки
не сверяются — модель недетерминирована, и тест на дословный текст
сломался бы на следующем же прогоне.

```bash
python -m evals.runner
```

36 кейсов: happy path, отказ по правам, no-source, prompt injection,
граница вопрос/действие, рискованные действия с подтверждением. 22 из
них проверяют **отказы** — то, чего система делать не должна.

| Метрика | Что означает | Порог |
|---|---|---|
| `task_success` | доля полностью пройденных кейсов | ≥ 85% |
| `intent_accuracy` | запрос ушёл в верную ветку графа | — |
| `tool_selection_accuracy` | выбран ожидаемый инструмент | — |
| `context_recall` | лучший найденный документ — нужный | — |
| `permission_safety` | недоступный роли документ не попал в выдачу | **100%** |
| `approval_compliance` | задача создана ровно там, где был approve | **100%** |

Две метрики блокирующие: при их падении runner завершается кодом 1, то
есть CI не пропустит релиз.

**`approval_compliance` проверяет побочный эффект, а не паузу.** «Граф
встал на interrupt» — это про управление; доказывать нужно, что задача
не была создана. Runner подменяет `execute_create_task` счётчиком и
сверяет число вызовов с ожиданием кейса. Ради этого создание задачи
вынесено из текста ответа в отдельную функцию: чтобы что-то измерить,
это сначала нужно сделать наблюдаемым.

**Датасет проверен на фальсифицируемость.** Зелёный scorecard ничего не
доказывает, если тест не способен упасть: в первой версии
`permission_safety` показывала 100% лишь потому, что список найденных
документов был пуст из-за бага в самом runner'е. Поэтому обе блокирующие
метрики проверены намеренной поломкой кода:

| Что сломано | Реакция датасета |
|---|---|
| `create_task` переведён на `risk_level="low"` | `approval_compliance` 91.7%, семь risky-кейсов красные |
| `permission_node` возвращает все документы | `permission_safety` 66.7%, конфиденциальные документы всплыли даже на нейтральных вопросах |

Обе поломки дают `RELEASE BLOCKED`.

Eval гоняется **без поднятого сервера** — обработчики FastAPI вызываются
как обычные функции. Это возможно потому, что логика лежит в графе и
модулях, а эндпоинты тонкие. Прогон стоит около 90 вызовов модели и
примерно пять минут.
...
Обращения к модели проходят через `generate_text()` с повторами:
провайдер отвечает `429` при превышении квоты, и это временная ошибка.
Повторяются только временные сбои (`429`, `503`, таймауты) с растущей
паузой 2 → 4 → 8 секунд; неверный ключ или битый промпт от повтора не
починятся, поэтому падают сразу.

Если попытки исчерпаны, решение принимает узел графа, а не слой доступа
к модели: `intent_node` откатывается в безопасное `"question"`,
`answer_node` — на сырой текст документа (он хуже читается, но не
галлюцинирует). В журнал пишется не только класс исключения, но и
причина: `type(e).__name__` без текста ошибки превращает разбор инцидента
в гадание.

Ветки, не зависящие от модели, при сбое провайдера продолжают работать —
детектор prompt injection и фильтр документов по роли выполняются в
Python, до всякого обращения к LLM.

## Готовность к эксплуатации

Проект — рабочий прототип. Пилота на реальной команде не проводилось,
поэтому ниже — план и механизмы, а не отчёт о внедрении.

### Release gate

Изменения проходят два барьера:

| Барьер | Что проверяет | Где |
|---|---|---|
| Быстрые тесты | детерминированные слои защиты | автоматически, CI на каждый push |
| Полный прогон eval | траекторию запроса целиком | вручную перед релизом |

Блокирующие метрики: `permission_safety = 100%` и
`approval_compliance = 100%`. При их падении `evals/runner.py`
завершается кодом 1.

Отдельно проверяется, что датасет **способен** упасть: обе метрики
протестированы намеренной поломкой кода (см. раздел Evaluation). Тест,
который не может стать красным, ничего не доказывает.

### Пилот

| Параметр | Значение |
|---|---|
| Аудитория | одна команда, 10–15 человек |
| Сценарии | вопросы по HR- и IT-политикам, проверка остатка отпуска |
| Длительность | две недели |
| Критерии успеха | ни одного нарушения контроля доступа; ни одного действия без approval; доля `no_source` ниже 20% |
| Что собираем | оценка «помогло / не помогло», случаи неверного источника, случаи отказа на законный вопрос |

Инструменты, пишущие во внешние системы, в пилот входят только в режиме
mock: реальная интеграция подключается после того, как approval-контур
отработает на живых пользователях.

### Реагирование на инцидент

Порядок действий — в [RUNBOOK.md](RUNBOOK.md). Ключевое решение:
при нарушении контроля безопасности инструмент выключается **до**
диагностики. Kill switch обратим одной командой, а выполненное действие
необратимо; при несимметричной стоимости ошибок выбирается дешёвая.

### Добавление новых инструментов

Инструмент не добавляется на ходу: карточка с честным `risk_level`,
функция исполнения, минимум два eval-кейса (happy path и отказ), обе
блокирующие метрики 100%. Инструмент без eval-кейсов в реестр не
попадает — иначе первым его поведение проверит пользователь.

### Чего не хватает для настоящего прода

| Область | Сейчас | Нужно |
|---|---|---|
| Состояние | в памяти процесса | Redis (лимиты), БД (kill switch), `PostgresSaver` (паузы) |
| Мониторинг | журнал событий | дашборд: latency, cost, доля отказов, частота `*_llm_error` |
| Алерты | нет | оповещение при всплеске `denied` и ошибок модели |
| Доступ к журналу | `/audit` открыт всем | закрыть ролью Admin |
| Аутентификация | заглушка на три пользователя | SSO |
| Источник документов | код | внешнее хранилище с версионированием и ingestion |
...
- **Answerability check — по подстроке.** Если модель ответила «в документе
  ...
  `has_answer`.
- **Ответ строится по одному документу.** Порог `RELEVANCE_THRESHOLD = 0.40`
  пропускает в выдачу лишние документы (три-четыре на запрос), но лучший по
  релевантности стабильно верный — `context_recall = 100%` на датасете.
  Reranking и объединение источников не реализованы, поэтому вопросы,
  ответ на которые собирается из двух policy сразу, не покрыты.
- **Автопроверки groundedness нет.** Grounded-промпт снижает риск
  галлюцинаций, но не гарантирует их отсутствие. Eval проверяет
  траекторию, а не качество формулировок; автоматическая сверка ответа с
  источником и LLM-as-judge — следующий шаг.
- **Eval-датасет ручной.** 36 кейсов покрывают основные ветки, но написаны
  руками. Прогон стоит около 90 вызовов модели и примерно пять минут,
  поэтому гоняется перед релизом, а не на каждое сохранение файла.
- **Классификатор намерения недетерминирован на пограничных запросах.**
  «Какая завтра погода» между прогонами относится то к вопросам, то к
  действиям; обе трактовки защитимы, но результат не воспроизводится.
  На безопасность это не влияет — несуществующий инструмент отсекается
  реестром, — но eval может «мигать» на одном-двух кейсах.
...
- **Событие `answer_delivered` для действий** пишется с `user_id`
  подтверждающего, а не запросившего: статистика по автору запроса
  искажается на approved-действиях.
